...
set CLASS_PATH_JARS=%JAR_JUNIT4%;%JAR_COMMONS_LOGGING%;%JAR_SPRING_TEST%;%JAR_SPRING_CORE%;%JAR_SPRING_BEANS%;%JAR_SPRING_CONTEXT%;%JAR_SPRING_AOP%;%JAR_SPRING_EXPRESSION%;%JAR_HAMCREST%;%PRJ_DIR%\LoulanDNS\target\classes;%PRJ_DIR%\LoulanDNS\target\test-classes;%PRJ_DIR%\lib\dnsjava-3.5.3.jar;%PRJ_DIR%\lib\slf4j-api-2.0.12.jar
...
if "%ARG1%"=="JUNIT-HELP" (
    java -cp %CLASS_PATH_JARS% %MAIN_CLASS%  --help
    exit /b 0
)
...
if "%ARG1%"=="ALL" (
    java -cp %CLASS_PATH_JARS% %MAIN_CLASS%  --scan-classpath %PRJ_DIR%\LoulanDNS\target\classes;%PRJ_DIR%\LoulanDNS\target\test-classes 
    exit /b 0
)

if "%ARG1%"=="PACKAGE" (
    java-cp %CLASS_PATH_JARS%  %MAIN_CLASS%  --select-package="%ARG2%"
    exit /b 0
)

if "%ARG1%"=="CLASS" (
    java -cp %CLASS_PATH_JARS%  %MAIN_CLASS%  "%ARG2%"
    exit /b 0
)
